...
func (pq *ParticipantQuery) QueryEvent() *EventQuery {
...
func (pq *ParticipantQuery) WithEvent(opts ...func(*EventQuery)) *ParticipantQuery {
	query := (&EventClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ParticipantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Participant, error) {
	...
	if pq.withUser != nil || pq.withEvent != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withEvent; query != nil {
		if err := pq.loadEvent(ctx, query, nodes, nil,
			func(n *Participant, e *Event) { n.Edges.Event = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ParticipantQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Participant, init func(*Participant), assign func(*Participant, *User)) error {
	...
	nodeids := make(map[int][]*Participant)
	for i := range nodes {
...
func (pq *ParticipantQuery) loadEvent(ctx context.Context, query *EventQuery, nodes []*Participant, init func(*Participant), assign func(*Participant, *Event)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Participant)
	for i := range nodes {
